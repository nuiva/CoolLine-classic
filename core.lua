...
--			[GetSpellInfo(55233) or "Vampiric Blood"] = 11,
	...
	updatelook = function()
		...
		for i = 0,8 do
			ticks[i+1] = createfs(ticks[i+1], tostring(i * 1.5), db.w * 1.5 / 12 * i)
		end
...
local function SetupIcon(frame, position, tthrot, active, fl)
	...
	frame:SetAlpha(active and db.activealpha or db.inactivealpha)
end

local function compareEndTimes(a,b)
	return a.endtime < b.endtime
end

local UpdateGCD
do
	local texture = CoolLine:CreateTexture()
	texture:SetColorTexture(.5,0,0,1)
	texture:SetDrawLayer("ARTWORK", 1)
	UpdateGCD = function()
		local remaining = 0
		local start, duration = GetSpellCooldown(2383)
		if start > 0 then
			remaining = start + duration - GetTime()
		end
		local width = remaining / 12 * db.w
		texture:ClearAllPoints()
		texture:SetSize(width, db.h)
		texture:SetPoint("LEFT", CoolLine, "LEFT", 0)
	end
end
...
local function OnUpdate(this, a1, ctime, dofl)
	...
	for _, frame in pairs(cooldowns) do
		...
		if remain > 12 then
			table.insert(overMaxTime, frame)
		elseif remain > -1 then
			SetupIcon(frame, db.w / 12. * remain, 0.01, true)
			if remain < 0 then
				frame:SetAlpha(remain + 1)
			end
		else
			ClearCooldown(frame)
		end
	end
	table.sort(overMaxTime, compareEndTimes)
	for i,frame in pairs(overMaxTime) do
		SetupIcon(frame, db.w + i * iconsize, 0.1, false)
	end
end
